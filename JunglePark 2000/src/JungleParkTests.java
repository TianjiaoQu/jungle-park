//////////////////// HEADER COMMENT ///////////////////////////////////////////////////////////////
//
// Title:           JungleParkTests.java is one of nine files for JunglePark 2000, which defines 
//                  tests to test the methods in JunglePark application
...
/**
 * This class defines tests to test the methods in JunglePark application
...
public class JungleParkTests extends JunglePark {
 ...
 // Create three tigers
    ...
    // Set t1 at position(200,200)
    t1.setPositionX(200);
    t1.setPositionY(200);
    // Set t2 at position(350,200)
    t2.setPositionX(350); // tiger1 is 150px away from tiger2
    t2.setPositionY(200);
    // Set t3 at position(250,200)
    t3.setPositionX(250); // tiger1 is 100px away from tiger2
    t3.setPositionY(200);
    ...
    if (!t1.isClose(t3, 100)) { // bug! isClose() should return true here
  ...
  public static boolean test1DeerScanForThreatMethod() {
    ...
    Tiger t1 = new Tiger(park);
    Tiger t2 = new Tiger(park);
    // Add the deer and tigers to listGUI
    park.listGUI.add(d);
    park.listGUI.add(t1);
    park.listGUI.add(t2);
    // Set deer at position(200,200)
    ...
    d.setPositionY(200);
    // Set first tiger at position(400,200)
    t1.setPositionX(400); // tiger1 is 200px away from deer
    t1.setPositionY(200);
    // Set second tiger at position(300,200)
    t2.setPositionX(300); // tiger2 is 100px away from deer
    t2.setPositionY(200);
  ...
  public static boolean test2DeerScanForThreatMethod() {
    boolean passed = true;
    // Create two deers and two tigers
    Deer d1 = new Deer(park);
    Deer d2 = new Deer(park);
    Tiger t1 = new Tiger(park);
    Tiger t2 = new Tiger(park);
    // Add the deers and tigers to listGUI
    park.listGUI.add(d1);
    park.listGUI.add(d1);
    park.listGUI.add(t1);
    park.listGUI.add(t2);
    ...
    d1.setPositionX(200);
    d1.setPositionY(200);
    // Set second deer at position(300,200)
    d2.setPositionX(300);
    d2.setPositionY(200);
    // Set first tiger at position(400,200)
    t1.setPositionX(400); // tiger1 is 200px away from deer1, 100px away from deer2
    t1.setPositionY(200);
    // Set second tiger at position(300,200)
    t2.setPositionX(380); // tiger is 180px away from deer1, 80px away from deer2
    t2.setPositionY(200);
    
    if (d1.scanForThreat(175)) { // bug! isClose() should return false here
      System.out.println("Deer's scanForThreat is returning true when it should return false.");
      passed = false;
    }
    if (!d2.scanForThreat(175)) { // bug! isClose() should return true here
      System.out.println("Deer's scanForThreat is returning false when it should return true.");
      passed = false;
    }
  ...
  public static boolean testTigerHopMethod() {
    ...
    // your hop() method
    // Create one deer and one tigers
    Deer d1 = new Deer(park);
    Tiger t = new Tiger(park);
    Deer d2 =new Deer(park);
    // Set the first deer at position(250,250)
    d1.setPositionX(250);
    d1.setPositionY(250);
    // Set the second deer at position(200,200)
    d2.setPositionX(200);
    d2.setPositionY(200);
    // Set the tiger at position(300,300) tiger is 70.71px away from deer1
    t.setPositionX(300);
    t.setPositionY(300);
    ...
    park.listGUI.add(d2);
    park.listGUI.add(t);
    t.hop(d1); // tiger hops on the first deer
    t.hop(d2); // tiger hops on the second deer
    if (t.getPositionX() != d2.getPositionX() && t.getPositionY() != d2.getPositionY()) {
      // tiger should move to the position of the deer1
      System.out.println("Tiger did not move correctly when hopping.");
      passed = false;
    }
    if (park.listGUI.contains(d2)) {
      // deer1 should be removed from the park
      System.out.println("Deers were not removed after being hopped on.");
      passed = false;
    }
    if (t.getDeerEatenCount() != 2) {
      // deerEatenCount should be incremented. 
